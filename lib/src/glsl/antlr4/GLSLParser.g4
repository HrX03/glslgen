...
finiteExpression:
	...
	//only the grouping expression allows the sequence expression since the sequence expression has
	// the lowest precendence and not putting it in parentheses would simply create a sequence
	// expression around the whole expression
	...
	//Note: diverges from the spec by not allowing a prefixExpression as an identifier array-type
	// function identfiers are handled by typeSpecifier
	...
	| left = finiteExpression op = (
		LT_OP
		| GT_OP
		| LE_OP
		| GE_OP
	) right = finiteExpression												# relationalExpression
	...
	| left = finiteExpression op = BITWISE_XOR_OP right = finiteExpression	#
		bitwiseExclusiveOrExpression
	| left = finiteExpression op = BITWISE_OR_OP right = finiteExpression #
		bitwiseInclusiveOrExpression
	| left = finiteExpression op = LOGICAL_AND_OP right = finiteExpression	# logicalAndExpression
	| left = finiteExpression op = LOGICAL_XOR_OP right = finiteExpression	#
		logicalExclusiveOrExpression
	| left = finiteExpression op = LOGICAL_OR_OP right = finiteExpression #
...
declaration:
	...
	| typeQualifier (
		variableNames += IDENTIFIER (
			COMMA variableNames += IDENTIFIER
		)*
...
parameterDeclaration:
	fullySpecifiedType (
		parameterName = IDENTIFIER arraySpecifier?
	)?;
...
storageQualifier:
	...
	| SUBROUTINE (
		...
		LPAREN typeNames += IDENTIFIER (
			COMMA typeNames += IDENTIFIER
		)* RPAREN
	)?
	| DEVICECOHERENT
	| QUEUEFAMILYCOHERENT
...
//needs duplicated rule parts like this or it becomes mutually left-recursive constant expressions
...
initializer:
	...
	| LBRACE (
		initializers += initializer (
			COMMA initializers += initializer
		)* COMMA?
	)? RBRACE;
...
forStatement:
	attribute? FOR LPAREN (
		...
		condition = expression
		| initCondition = iterationCondition
	)? SEMICOLON incrementer = expression? RPAREN loopBody = statement;
